    ...
    <section className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 mb-4">
        <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
          ...
          <span>Pengalaman (Kerja / Magang / Organisasi)</span>
        </h2>
        <button type="button" onClick={addExperience} className="flex items-center justify-center gap-1 px-3 py-1.5 text-sm font-medium text-blue-600 bg-blue-50 rounded-lg hover:bg-blue-100 transition-colors whitespace-nowrap">
      ...
      {data.experiences.length === 0 ? (
        <p className="text-gray-500 text-center py-8 text-sm">
          Belum ada pengalaman. Kamu bisa menambahkan kerja part-time, magang, PKL, proyek sekolah, atau kepanitiaan/organisasi. Klik tombol <strong>\"Tambah\"</strong> untuk mulai.
        </p>
      ) : (
          ...
          {data.experiences.map((exp, index) => (
            <div key={exp.id} className="p-4 bg-gray-50 rounded-lg border border-gray-200">
              <div className="flex items-center justify-between mb-4">
                ...
                <button type="button" onClick={() => removeExperience(exp.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="Contoh: Software Developer, Project Manager, Content Writer"
                  />
                </div>
                ...
                <div>
                  ...
                  <RichTextEditor value={exp.description} onChange={(value) => updateExperience(exp.id, "description", value)} placeholder="Jelaskan tanggung jawab dan pencapaian Anda..." />
                </div>
              </div>
            </div>
          ))}
